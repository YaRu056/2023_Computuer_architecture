...
class BitPlanesTest extends AnyFlatSpec with ChiselScalatestTester {
  behavior.of("Single Cycle CPU")
  it should "Perform CLZ" in {
    test(new TestTopModule("Abit_planes_slicingAsm.asmbin")).withAnnotations(TestAnnotations.annos) { c =>
      for (i <- 1 to 500) {
        c.clock.step()
        c.io.mem_debug_read_address.poke((i * 4).U) // Avoid timeout
      }
      ...
      c.clock.step()
      c.io.regs_debug_read_data.expect(0x18.U)
      c.io.regs_debug_read_address.poke(19.U) // s3
      c.clock.step()
      c.io.regs_debug_read_data.expect(0x07.U)
